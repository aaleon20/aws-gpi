...
class DynamoAccessor:
    ...
    def get_data_from_dynamo(self, userid):
        response = self.table.query(KeyConditionExpression=Key('userid').eq(userid))
        return response["Items"][0] if any(response["Items"]) else None

def lambda_handler(event, context):
    dynamo_backend = DynamoAccessor(DYNAMO_BD)
    db_element = dynamo_backend.get_data_from_dynamo(event['userid'])
    return db_element
